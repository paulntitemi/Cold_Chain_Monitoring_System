 ...
 *
 * Copied verbatim from /web/src/lib/sigv4.ts — any change must land in both
 * apps or the two surfaces drift.
 */
    ...
    sortedHeaderKeys
      .map(
        (k) =>
          `${k}:${(
            baseHeaders[k] ??
            baseHeaders[Object.keys(baseHeaders).find((h) => h.toLowerCase() === k) ?? k] ??
            ''
          ).trim()}`,
      )
      .join('\n') + '\n';
